...
class GraphWithStructuredmemory:
    ...
    def llm_agent_with_memory(self, state: MemoryState) -> MemoryState:
        ...
        recall_str = (
                "<recall_memory>\n" + "\n".join(str(state["recall_memories"])) + "\n</recall_memory>"
        )
    ...
    def load_memories(state: MemoryState, config: RunnableConfig) -> MemoryState:
        ...
        recall_memories = GraphWithStructuredmemory.memory.search_memories.invoke(convo_str, config)
        return {
